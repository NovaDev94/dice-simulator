...
class DiceClient(object):
    ...
    def start(self, minimum_balance, maximum_balance):
        ...
        per_slot = 0.5
        slot_number = int(math.ceil((
            self.maximum_balance - self.balance) / per_slot))
        self.yolo = [0] + [
            int(per_slot * 100) for x in xrange(slot_number)] + [0]

        processor = self._process
        self.result = {}
        while not self.will_stop and self.yolo:
            processor()
    ...
    @property
    def front_and_rear(self):
        if not self.yolo:
            return 0
        if len(self.yolo) > 1:
            return self.yolo[0] + self.yolo[-1]
        return self.yolo[0]

    def remove_both_ends(self):
        if not self.yolo:
            return
        if len(self.yolo) > 1:
            del self.yolo[-1]
    ...
    def _process(self):
        ...
        if is_win:
            ...
            self.yolo.append(int(round(self.last_bet * 100)))
        # bet_amount = min(new_balance, bet_amount)
        bet_amount = utils.to_2(self.front_and_rear / 100.)
        # print sum(self.yolo), self.yolo
